...
const BuyerLogin: React.FC<BuyerLoginProps> = ({ onLogin, onBack }) => {
  ...
  const handleAuth = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (err.message && (err.message.includes("Failed to fetch") || err.message.includes("Network request failed"))) {
        ...
        setTimeout(() => {
            onLogin(username || 'Demo Buyer', email || 'demo@buyer.com');
        }, 1500);
      } else {
        setError(getFriendlyErrorMessage(err.message || 'Authentication failed'));
      }
    } finally {
      setLoading(false);
    }
    ...
    <div className="container mx-auto px-4 py-12 flex items-center justify-center animate-in fade-in zoom-in-95 duration-500">
      <div className="bg-white p-8 md:p-12 rounded-[40px] shadow-2xl border border-slate-100 max-w-2xl w-full">
        ...
        <div className="mb-8 text-center">
          <h2 className="text-4xl font-black text-slate-900 mb-3 tracking-tight">Buyer Portal</h2>
          <p className="text-slate-500 font-medium">Access verified suppliers and AI-powered sourcing.</p>
        </div>
        ...
        <form onSubmit={handleAuth} className="space-y-6">
          {mode === 'register' ? (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 animate-in slide-in-from-top-2 duration-300">
              <div className="col-span-2">
                <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2 ml-4">Full Name / Company</label>
                <input 
                  required
                  type="text" 
                  ...
                  placeholder="e.g. Global Sourcing Inc."
                  value={username}
              ...
              <div className="col-span-2">
                <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2 ml-4">Job Role</label>
                <input 
                  required
                  type="text" 
                  className="w-full px-6 py-4 bg-slate-50 border-transparent focus:bg-white focus:ring-2 focus:ring-blue-500 rounded-2xl outline-none transition-all font-medium"
          ...
          <button 
            type="submit" 
            disabled={loading}
            className="w-full py-5 bg-blue-600 text-white rounded-3xl font-black uppercase tracking-widest hover:bg-slate-900 transition-all shadow-xl shadow-blue-500/20 active:scale-95 mt-4 disabled:opacity-50 disabled:cursor-not-allowed"
          >
            {loading ? 'Processing...' : mode === 'signin' ? 'Sign In to Portal' : 'Register Account'}
          </button>
        </form>

        <p className="mt-8 text-center text-slate-400 text-[10px] font-black uppercase tracking-widest">
          Trusted by 500+ global procurement agents.
        </p>
      </div>
    </div>
  );
